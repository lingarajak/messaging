...
app.post('/v1/ai/summarize', async (req, res) => {
  ...
  const completion = await openai.chat.completions.create({
    model: 'gpt-4o-mini',
    messages: [{ role: 'system', content: 'Summarize this chat in 3 bullet points with action items' }, { role: 'user', content: text }],
    max_tokens: 200
  });
...
app.post('/v1/ai/transcribe', async (req, res) => {
  ...
  await redis.hset(`msg:${msgId}`, 'transcription', 'Processing...');
